...
class AuthRepositoryImpl implements AuthRepository {
  ...
  Future<SendOtp> sendOtp(String email) async {
    ...
    try {
      ...
      Logger.info(
        '✨ Parsed SendOtp Model -> status: "${sendOtpResponse.status}", message: "${sendOtpResponse.message}"',
        'AuthRepository',
      );
      ...
      Logger.error(
        '❌ [API ERROR - ApiException] Status: ${e.statusCode}',
        e.message,
        null,
        'AuthRepository',
      );
      if (e.errorData != null) {
        ...
        if (e.errorData is Map<String, dynamic>) {
          ...
          try {
            final jsonMap =
                jsonDecode(e.errorData as String) as Map<String, dynamic>;
  ...
  Future<LoginSendOtp> loginSendOtp(String email) async {
    ...
    try {
      ...
      Logger.info(
        '✨ Parsed LoginSendOtp Model -> status: "${loginSendOtpResponse.status}", message: "${loginSendOtpResponse.message}"',
        'AuthRepository',
      );
  ...
  Future<ResendOtp> resendOtp(String email) async {
    ...
    try {
      ...
      Logger.info(
        '✨ Parsed ResendOtp Model -> status: "${resendOtpResponse.status}", message: "${resendOtpResponse.message}"',
        'AuthRepository',
      );
  ...
  Future<VerifyOtp> verifyOtp(String email, String code) async {
    ...
    try {
      ...
      Logger.info(
        '✨ Parsed VerifyOtp Model -> status: "${verifyOtpResponse.status}", message: "${verifyOtpResponse.message}", registration_token: "${verifyOtpResponse.data?.registrationToken}"',
        'AuthRepository',
      );
  ...
  Future<LoginVerifyOtp> loginVerifyOtp(String email, String code) async {
    ...
    try {
      ...
      Logger.info(
        '✨ Parsed LoginVerifyOtp Model -> status: "${loginVerifyOtpResponse.status}", message: "${loginVerifyOtpResponse.message}", auth_token: "${loginVerifyOtpResponse.data?.authToken}", user_id: ${loginVerifyOtpResponse.data?.userId}',
        'AuthRepository',
      );
      ...
      if (loginVerifyOtpResponse.status == 'success') {
        ...
        await _hiveService.settingsBox.put('auth_user_email', email);
        Logger.info(
          '🔒 Securely stored auth_token ("$token") and user_id ($userId) in local Hive storage',
          'AuthRepository',
        );
      }
...
class MockAuthRepository implements AuthRepository {
  ...
  Future<SendOtp> sendOtp(String email) async {
    Logger.info(
      '[MockAuth] Requesting OTP code to be sent to: $email',
      'MockAuthRepository',
    );
    await Future.delayed(const Duration(milliseconds: 1000));
    Logger.info(
      '[MockAuth] OTP code sent successfully. Use code: $_mockOtp',
      'MockAuthRepository',
    );
  ...
  Future<LoginSendOtp> loginSendOtp(String email) async {
    Logger.info(
      '[MockAuth] Requesting login OTP code to be sent to: $email',
      'MockAuthRepository',
    );
    await Future.delayed(const Duration(milliseconds: 1000));
    Logger.info(
      '[MockAuth] Login OTP code sent successfully. Use code: $_mockOtp',
      'MockAuthRepository',
    );
  ...
  Future<ResendOtp> resendOtp(String email) async {
    Logger.info(
      '[MockAuth] Resending OTP code to: $email',
      'MockAuthRepository',
    );
    await Future.delayed(const Duration(milliseconds: 1000));
    Logger.info(
      '[MockAuth] OTP code resent successfully. Use code: $_mockOtp',
      'MockAuthRepository',
    );
  ...
  Future<VerifyOtp> verifyOtp(String email, String code) async {
    Logger.info(
      '[MockAuth] Verifying OTP code: "$code" for email: $email',
      'MockAuthRepository',
    );
    ...
    if (code != _mockOtp) {
      Logger.warning(
        '[MockAuth] Verification failed. Invalid code "$code" entered for $email',
        'MockAuthRepository',
      );
      return VerifyOtp(
        status: 'error',
    ...
    Logger.info(
      '[MockAuth] Verification successful for $email',
      'MockAuthRepository',
    );
  ...
  Future<LoginVerifyOtp> loginVerifyOtp(String email, String code) async {
    Logger.info(
      '[MockAuth] Verifying login OTP code: "$code" for email: $email',
      'MockAuthRepository',
    );
    await Future.delayed(const Duration(milliseconds: 1200));
